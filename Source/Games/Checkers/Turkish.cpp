...
namespace Checkers
{
  ...
  void TurkChecker::CollectMoves(const MainPosition& p, const Location& l, Moves& moves) const noexcept
  {
    const TurkCheckersPosition& pos = dynamic_cast<const TurkCheckersPosition&>(p);
    const int dy = pos.GetPiece(l).IsColor(PieceColor::White) ? -1 : 1;
    pos.AddIfLegal(moves, l, l + Offset(1, dy));                          // check for slide moves
    pos.AddIfLegal(moves, l, l + Offset(0, dy));
    pos.AddIfLegal(moves, l, l + Offset(-1, dy));
    pos.AddIfLegalJump(moves, false, Actions{}, pos.GetPiece(l), l);        // check for jump moves
  }

  void TurkCheckersPosition::SetStartingPosition() noexcept
  {
    for (Coordinate j = 1; j < sizeY_ / 4 + 1; j++)
    {
      for (Coordinate i = 0; i < sizeX_; i++)
      {
        SetPiece(Location(BoardPart::Main, i, j             ), TurkCheckersPiece::TurkCheckersPieceB);
        SetPiece(Location(BoardPart::Main, i, sizeY_ - 1 - j), TurkCheckersPiece::TurkCheckersPieceW);
      }
    }
  }

// p is the piece that is moving, but it doesn't necessarily come from fr - it could be a continuation jump
  bool TurkCheckersPosition::AddIfLegalJump(Moves& m, bool longjumps, const Actions& a, const Piece& p, const Location& fr) const noexcept
  {
  ...
  const PieceMapP& TurkCheckersGame::GetPieces(const VariantChosen& v) noexcept
  {
