...
namespace Template
{
  ...
  {
  private:
    TemplatePiece(const Kind* k, const Color* c, UINT l, UINT s) : Piece(k, c, l, l, s) {}
  ...
  {
  public:
    TemplatePosition(unsigned int x, unsigned int y);
  ...
  {
  public:
    TemplateLayout(unsigned int x, unsigned int y) : MainLayout(Dimension(x, y, BoardStartX, BoardStartY, FieldSizeX, FieldSizeY)) {}
  ...
  {
  public:
    TemplateTakenLayout(unsigned int x, unsigned int y) :
  ...
  {
  public:
    TemplateStockLayout(unsigned int x, unsigned int y) :
  ...
  private:
    TemplateGame(TemplatePosition* p, TakenPosition* t, StockPosition* s,
      TemplateLayout* l, TemplateTakenLayout* tl, TemplateStockLayout* sl) : Game{p,t,s,l,tl,sl}
    {
      AddToStock(Location(0, 0), &TemplatePiece::TemplatePieceW);
      AddToStock(Location(1, 0), &TemplatePiece::TemplatePieceB);
    }
  public:
    TemplateGame(unsigned int x, unsigned int y) : TemplateGame(
